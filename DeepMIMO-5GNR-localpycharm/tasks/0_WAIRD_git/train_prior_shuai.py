...
def run(isTrain, data_loader, model, epoch, device, imageSize, class_z, class_num):#一个batch运行的程序，不区分train/test
    ...
    # for i, (channels, UE_x, UE_y, UE_distance, UE_gain) in enumerate(data_loader):#change according to dataset
    for i, (channels, z, UE_x, UE_y, UE_distance, UE_gain, dod_phi, dod_theta, ADP, combineCFRADP, z_CFR, z_ADP, z_CFR_manmadearea) in enumerate(data_loader):  # change according to dataset; (channel, z, UE_x, UE_y)
        p_ueloc_x = []
        p_ueloc_y = []
        a = torch.concat([UE_distance, UE_gain, dod_phi, dod_theta], dim = 1)

        #change data to gpu
        channels = channels.to(device, non_blocking=True)
        z = z.to(device, non_blocking=True)
        z_CFR = z_CFR.to(device, non_blocking=True)
        z_ADP = z_ADP.to(device, non_blocking=True)
        z_CFR_manmadearea = z_CFR_manmadearea.to(device, non_blocking=True)
        ...
        # UE_gain = UE_gain.to(device, non_blocking=True)
        ...
        if isTrain:
            # p_UEloc = model.update(z, channels, y, a, epoch, device)#perfect ADP
            p_UEloc = model.update(z, channels, y, ADP, epoch, device)
            # p_UEloc = model.update(z_CFR, channels, y, ADP, epoch, device) #只是试一试
            # p_UEloc = model.update(z, combineCFRADP, y, epoch, device)
            # p_UEloc = model.update(z_CFR, channels, y, ADP, epoch, device) #CFR用我们的分类方式 DS_AE_CFRorADP
            # p_UEloc = model.update(z_ADP, channels, y, ADP, epoch, device) #ADP用我们的分类方式 DS_AE_CFRorADP
            # p_UEloc = model.update(z_CFR_manmadearea, channels, y, ADP, epoch, device)#CFR用人造的分类方式 DS_AE_CFR，先验拿不到
            # p_UEloc = model.update(z_CFR, channels, y, ADP, epoch, device)  # CFRADP用我们的分类方式 DS_AE_CFRADP，结果不大好
            print(
                # 'Epoch{}:[{}/{}({:.0f}%)]'.format(epoch,trainedsamples,allsamples,100*trainedsamples/allsamples))
                f'{setStr} Epoch: [{epoch}][{i}/{len(data_loader)}] ---- offset {p_UEloc["loss"]:.4f}meters^2')
        if not isTrain:
            # y_hat = model.predict(z, channels, a, device) #perfect ADP
            y_hat = model.predict(z, channels, ADP, device)
            # y_hat = model.predict(z_CFR, channels, ADP, device)#只是试一试
            # y_hat = model.predict(z, combineCFRADP, device)
            # y_hat = model.predict(z_CFR, channels, ADP, device)#CFR用我们的分类方式 DS_AE_CFRorADP
            # y_hat = model.predict(z_ADP, channels, ADP, device)#ADP用我们的分类方式 DS_AE_CFRorADP
            # y_hat = model.predict(z_CFR_manmadearea, channels, ADP, device)#CFR用人造的分类方式 DS_AE_CFR
            ...
            print(
                # 'Epoch{}:[{}/{}({:.0f}%)]'.format(epoch,trainedsamples,allsamples,100*trainedsamples/allsamples))
                f'{setStr} Epoch: [{epoch}][{i}/{len(data_loader)}] ---- offset {offset:.4f}meters^2')
...
def main():
    ...
    # model = DS_AE_CFRperfectADP(hparams, device).to(device, non_blocking=True)  # 多头+cnn
    # model = CNN_prior(hparams, device).to(device, non_blocking=True)
    # model = DS_AE_bothmulti(hparams, device, class_z, class_num).to(device, non_blocking=True)
    ...
    # model = CNN_prior_CFRADP(hparams, device).to(device, non_blocking=True) #CFR和ADP分别输入benchmark生成特征结合
    # model = DS_AE_CFRorADP(hparams, device).to(device, non_blocking=True)
    # model = DS_AE_manmade(hparams, device).to(device, non_blocking=True) #只
    #CFR和ADP结合成一张输入图片
    # model = DS_AE_combineCFRADP(hparams, device).to(device, non_blocking=True)
    #加入ViT
    # model = effiformerv2_DS_AE_CFRADP(hparams, device).to(device, non_blocking=True)#在DS_AE_CFRADP结构上只替换encoder
    # model = effiformerv2_DS_AE_CFRperfectADP(hparams, device).to(device, non_blocking=True)


    resultsfolder = os.path.join(savedatadir, case, 'CFRADP')
    # resultsfolder = os.path.join(savedatadir, case, 'CFRADP', 'densenet')
    # resultsfolder = os.path.join(savedatadir, case, 'CFRADP', 'ViT')#ViT
    # resultsfolder = os.path.join(savedatadir, case, 'CFR') #DS_AE_manmade的CFR
    # resultsfolder = os.path.join(savedatadir, case, 'ADP')  # DS_AE_manmade的ADP


    if os.path.exists(os.path.join('results', savedatadir, case)):
        pass
    else:
        # 创建文件夹
        os.mkdir(os.path.join('results', savedatadir, case))#避免每次操作都手动生成文件夹
    ...
    Train_loss_loc = os.path.join(resultsfolder,'Train_loss_' + str(model.__class__.__name__) + '_' + str(sim_threshold) + '_' + str(sim_threshold_betweenTemp) + '_' + str(lr) + '_' + str(batchSize) + '_' + str(hparams['lr']) + '_' + str(hparams['temperature']) + '.csv')
    Test_loss_loc = os.path.join(resultsfolder,'Test_loss_' + str(model.__class__.__name__) + '_' + str(sim_threshold) + '_' + str(sim_threshold_betweenTemp) + '_' + str(lr) + '_' + str(batchSize) + '_' + str(hparams['lr']) + '_' + str(hparams['temperature']) + '.csv')
    ...
    for epoch in range(1, epochs + 1):
        ...
        if (not evaluation):
            LossT = run(True, train_loader, model, epoch, device, imageSize, class_z, class_num)
            Train_Loss.append(LossT)
            LossV, py_hat1, py1 = run(False, valid_loader, model, epoch, device, imageSize, class_z, class_num)
        ...
        if highestacc > LossV:
            ...
            if not os.path.exists(model_path):
                os.mkdir(model_path)
            # torch.save(model.state_dict(), os.path.join(model_path, modelname + ".pt"))
            print("\t Weight Saved")
        ...
        if epoch == epoch_stop or epoch % 50 == 0:
            ...
            # print(df_y_hat)
            df_y_hat.to_csv(os.path.join(resultsfolder, 'df_y_hat_' + str(model.__class__.__name__) + '_' + str(sim_threshold) + '_' + str(sim_threshold_betweenTemp) + '_' + str(lr) + '_' + str(batchSize) + '_' + str(hparams['lr']) + '_' + str(hparams['temperature']) + '.csv'), index=False, header=False)
            # 将目前最好的一次testloss的每一个bs的数据对应的xy保存以后续计算CDF
            # print(df_y)
            df_y.to_csv(os.path.join(resultsfolder, 'df_y_' + str(model.__class__.__name__) + '_' + str(sim_threshold) + '_' + str(sim_threshold_betweenTemp) + '_' + str(lr) + '_' + str(batchSize) + '_' + str(hparams['lr']) + '_' + str(hparams['temperature']) + '.csv'), index=False, header=False)
